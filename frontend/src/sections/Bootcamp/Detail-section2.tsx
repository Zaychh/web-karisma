import { useEffect, useState } from "react";

import kuning from "../../assets/Foto1.png";
import biru from "../../assets/Foto3.png";
import ungu from "../../assets/Foto6.png";
import merah from "../../assets/Foto7.png";
import hijau from "../../assets/hijau.png";

const imageList = [kuning, biru, ungu, hijau, merah];

interface Course {
  program_id: number;
  title: string;
}

const DetailSection2 = ({ course }: { course: Course }) => {
  const [facts, setFacts] = useState<string[]>([]);

  useEffect(() => {
    if (!course) return;
    fetch(`/api/program/${course.program_id}/facts`)
      .then((res) => res.json())
      .then((data) => setFacts(data))
      .catch((err) => console.error("Gagal ambil fakta:", err));
  }, [course]);

  if (!course) return null;

  const factsToRender = facts.length > 0 ? facts : Array(5).fill("Facts belum diisi!");

  return (
    <section className="bg-abyssal text-white py-16 px-6 md:px-16 font-poppins">
      <h2 className="text-2xl md:text-3xl font-bold text-center mb-10">
        5 Fakta Menarik Tentang Karir <br className="block md:hidden" />
        <span className="text-rosegold">{course.title}</span>
      </h2>

      {/* Desktop */}
      <div
        className="hidden md:grid grid-cols-4 gap-4 max-w-6xl mx-auto group"
        style={{
          gridTemplateRows: "repeat(2, 1fr)",
          gridTemplateAreas: `
            "card1 card2 card3 card5"
            "card1 card4 card4 card5"
          `,
          height: "520px",
        }}
      >
        {factsToRender.map((text, i) => {
          const areaName = `card${i + 1}`;
          const image = imageList[i % imageList.length];
          return (
            <div
              key={i}
              style={{ gridArea: areaName }}
              className="relative rounded-xl overflow-hidden w-full h-full transition duration-300 group-hover:opacity-40 hover:opacity-100 hover:scale-[1.02]"
            >
              <img
                src={image}
                alt={text}
                className="w-full h-full object-cover"
              />
              <div className="absolute inset-0 flex items-start justify-start p-4">
                <p
                  className={`text-sm md:text-lg font-bold leading-snug text-white ${
                    facts.length === 0 ? "italic text-gray-300" : ""
                  }`}
                >
                  {text}
                </p>
              </div>
            </div>
          );
        })}
      </div>

      {/* Mobile */}
      <div className="grid grid-cols-2 gap-4 md:hidden max-w-lg mx-auto">
        {factsToRender.map((text, i) => {
          const image = imageList[i % imageList.length];
          return (
            <div
              key={i}
              className="relative rounded-xl overflow-hidden w-full h-[180px] transition duration-300 group-hover:opacity-40 hover:opacity-100 hover:scale-[1.02]"
            >
              <img
                src={image}
                alt={text}
                className="w-full h-full object-cover"
              />
              <div className="absolute inset-0 flex items-start justify-start p-2">
                <p
                  className={`text-sm font-semibold leading-snug text-white ${
                    facts.length === 0 ? "italic text-gray-300" : ""
                  }`}
                >
                  {text}
                </p>
              </div>
            </div>
          );
        })}
      </div>
    </section>
  );
};

export default DetailSection2;
