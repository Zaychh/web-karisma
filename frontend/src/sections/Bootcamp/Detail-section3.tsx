...
const logoList = [JobstreetLogo, IndeedLogo, GlassdoorLogo, GlintsLogo, KalibrLogo];
...
const DetailSection3 = ({ course }: { course: Course }) => {
  const [jobs, setJobs] = useState<Job[]>([]);

  useEffect(() => {
    if (!course) return;
    fetch(`/api/program/${course.program_id}/jobs`)
      .then((res) => res.json())
      .then((data) => setJobs(data))
      .catch((err) => console.error("Gagal ambil jobs:", err));
  }, [course]);

  if (!course) return null;

  const jobsToRender = jobs.length > 0 ? jobs : Array(5).fill({
    company: "",
    position: "-",
    type: "-",
    salary: "-",
        ...
        <section className="bg-ashh text-white py-20 px-6 md:px-16">
            <h2 className="text-xl md:text-2xl font-semibold text-center mb-10">
                ...
                <span className="hidden md:block">
                    Peluang karir <span className="text-rosegold">{course.career_title}</span> menjanjikan
                    dengan jenjang yang jelas, dibutuhkan perusahaan dalam jangka panjang.
                </span>

                {/* Mobile version */}
                <span className="block md:hidden">
                    Peluang karir <span className="text-rosegold">{course.career_title}</span>
                </span>
            </h2>
            ...
            >
              <div className="flex items-center gap-8">
                <img src={logoList[idx % logoList.length]} alt={job.company || "Logo"} className="w-24 h-auto object-contain" />
          ...
          >
            <div className="w-full flex justify-center mb-4">
              <img src={logoList[idx % logoList.length]} alt={job.company || "Logo"} className="w-[80%] h-auto object-contain" />
            </div>
