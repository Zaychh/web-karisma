...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, "uploads/"); // ⬅ tetap di folder uploads/
  },
  filename: (req, file, cb) => {
    // ✅ Fix: gunakan backticks untuk template literal
...
router.get("/me", verifyToken, async (req, res) => {
  try {
    ...
    const user = rows[0];
    console.log("🔍 Backend - User data from DB:", user);
    console.log("🖼️ Backend - Image field:", user.image);
